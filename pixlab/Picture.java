...
public class Picture extends SimplePicture 
{
  ...
  public  void grayScale()
  {
    ...
    for(Pixel[] row: image)
    {
      for(Pixel p: row)
      {
        int average = (p.getGreen()+ p.getBlue() + p.getRed())/3;
        p.setRed(average);
        p.setBlue(average);
        p.setGreen(average);
      }  
  ...
  public void mirrorHorizontalBotToTop()
  {
   ...
   for(int row = pixels.length/2; row<pixels.length; row++)
   {
       ...
       {
           botPixel = pixels[pixels[0].length - row -1][col];
           topPixel = pixels[row][col];
  ...
  public void mirrorGull()
  {
      ...
      Pixel rightPixel = null;
      Pixel[][]pixels = this.getPixels2D();
      for(int row = 237; row< 327; row++)
      {
          for(int col = 228; col<342; col++)
          {
             leftPixel = pixels[row][col];
             rightPixel = pixels[row][342 - col +342];
             rightPixel.setColor(leftPixel.getColor());
          }
      }
  }
